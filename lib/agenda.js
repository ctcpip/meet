...
async function fetchAgendaItems (client, repos, agendaLabel) {
  ...
  for (const r of uniqueRepos) {
    ...
    let _agendaPrs = await client.paginate('GET /repos/{owner}/{repo}/pulls', {
      owner: r.owner,
      repo: r.repo,
      state: 'open',
      labels: agendaLabel,
    ...
    _agendaPrs = _agendaPrs.filter(pr => pr.labels.find(label => label.name === agendaLabel) &&
    !agendaIssues.find((i) => i.url === pr.url))

    console.log(`Fetching PRs for ${r.owner}/${r.repo}: Found ${_agendaPrs.length}`)

    for (const pr of _agendaPrs) {
    // workaround for flaky GH API/SDK behavior where sometimes the issue endpoint loads PRs
      if (!agendaIssues.find((ii) => ii.url === pr.url) &&
        pr.labels.find(label => label.name === agendaLabel)) {
